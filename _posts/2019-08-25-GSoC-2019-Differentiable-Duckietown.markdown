...
## Rendering the view
`renderobs(...)` is used to render was duckiebot sees. We use differentiable [RayTracer.jl](https://github.com/avik-pal/RayTracer.jl) for this purpose. For rendering we first need to define a camera model. Camera needs to know where the bot is looking from & at, dimensions of image, fied of view, focal length, and up vector.
```julia
x, y, z = sim.cur_pos
